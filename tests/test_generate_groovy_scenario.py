...
def test_main_valid_json(mocker, json_scenario):
    mocker.patch("generate_scenario.open", mock_open(read_data=json.dumps(json_scenario)))

    mock_generate_groovy_file = mocker.patch("generate_scenario.generate_groovy_file")
...
def test_main_invalid_json(mocker, mock_logger):
    ...
    mock_sys_exit = mocker.patch("generate_scenario.exit")
    mock_get_logger = mocker.patch("generate_scenario.get_logger")
...
def test_generate_groovy_file(mock_logger, mock_builder, json_scenario):
    with patch("generate_scenario.RCGroovyScenarioBuilder", return_value=mock_builder):
