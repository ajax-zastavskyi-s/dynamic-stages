...
def getStages() {
    ...
    return [
        [
            name: "Deploy a911-svc 1.122.0-7684.RELEASE",
            steps: {
                script {
                    parallel (
                        "a911-svc 1.122.0-7684.RELEASE": {
                            dynamicStagesResults = getDynamicStagesResults()

                            def serviceVersionFromPattern = rc_testing.getLatestServiceVersionByPattern(
                                serviceName="a911-svc",
                                serviceVersionPattern="1.122.0-7684.RELEASE"
                            )
                            ...
                            if (dynamicStagesResults['deploy_a911_svc_passed'] == false) {
                                saveFailedStages("Deploy a911-svc with version 1.122.0-7684.RELEASE")
                            }
                ...
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        rc_testing.runBDDTests(
                            marks='Empty',
                            test_plan_name='RC [a911-svc 1.122.0-7684.RELEASE]',
