...
def getStages() {
    ...
    return [
        [
            name: "Deploy user-svc 1.35.0*.RELEASE | a911-svc 1.133.0*.RELEASE | cms-gw-svc 1.42.0*.RELEASE | hub-svc 1.44.0*.RELEASE | cloud-signaling-svc 1.112.0*.RELEASE",
            steps: {
                script {
                    parallel (
                        "user-svc 1.35.0*.RELEASE": {
                            dynamicStagesResults = getDynamicStagesResults()

                            def serviceVersionFromPattern = rc_testing.getLatestServiceVersionByPattern(
                                serviceName="user-svc",
                                serviceVersionPattern="1.35.0*.RELEASE"
                            )
                            ...
                            if (dynamicStagesResults['deploy_user_svc_passed'] == false) {
                                saveFailedStages("Deploy user-svc with version 1.35.0*.RELEASE")
                            }
                        ...
                        "cms-gw-svc 1.42.0*.RELEASE": {
                            ...
                            if (dynamicStagesResults['deploy_cms_gw_svc_passed'] == false) {
                                saveFailedStages("Deploy cms-gw-svc with version 1.42.0*.RELEASE")
                            }
        
                            env.dynamicStagesResults = groovy.json.JsonOutput.toJson(dynamicStagesResults)
                        },
                        "hub-svc 1.44.0*.RELEASE": {
                            dynamicStagesResults = getDynamicStagesResults()

                            def serviceVersionFromPattern = rc_testing.getLatestServiceVersionByPattern(
                                serviceName="hub-svc",
                                serviceVersionPattern="1.44.0*.RELEASE"
                            )
                            ...
                            if (dynamicStagesResults['deploy_hub_svc_passed'] == false) {
                                saveFailedStages("Deploy hub-svc with version 1.44.0*.RELEASE")
                            }
        
                            env.dynamicStagesResults = groovy.json.JsonOutput.toJson(dynamicStagesResults)
                        },
                        "cloud-signaling-svc 1.112.0*.RELEASE": {
                            dynamicStagesResults = getDynamicStagesResults()

                            def serviceVersionFromPattern = rc_testing.getLatestServiceVersionByPattern(
                                serviceName="cloud-signaling-svc",
                                serviceVersionPattern="1.112.0*.RELEASE"
                            )
                            ...
                            if (dynamicStagesResults['deploy_cloud_signaling_svc_passed'] == false) {
                                saveFailedStages("Deploy cloud-signaling-svc with version 1.112.0*.RELEASE")
                            }
                ...
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        rc_testing.runBDDTests(
                            marks='Empty',
                            test_plan_name='RC [user-svc 1.35.0 | a911-svc 1.133.0 | cms-gw-svc 1.42.0 | hub-svc 1.44.0 | cloud-signaling-svc 1.112.0]',
        ...
        [
            name: "Deploy company-svc 1.101.0*.RELEASE",
            steps: {
                script {
                    parallel (
                        "company-svc 1.101.0*.RELEASE": {
                            dynamicStagesResults = getDynamicStagesResults()
                            ...
                            if (dynamicStagesResults['deploy_company_svc_passed'] == false) {
                                saveFailedStages("Deploy company-svc with version 1.101.0*.RELEASE")
                            }
            ...
            steps: {
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        rc_testing.runBDDTests(
                            marks='Empty',
                            test_plan_name='RC [user-svc 1.35.0 | a911-svc 1.133.0 | cms-gw-svc 1.42.0 | hub-svc 1.44.0 | cloud-signaling-svc 1.112.0 | company-svc 1.101.0]',
                            test_plan_description='RC Testing'
                        )
                    }
                    else {
        ...
        [
            name: "Deploy communication-svc 1.76.0*.RELEASE",
            steps: {
                script {
                    parallel (
                        "communication-svc 1.76.0*.RELEASE": {
                            dynamicStagesResults = getDynamicStagesResults()

                            def serviceVersionFromPattern = rc_testing.getLatestServiceVersionByPattern(
                                serviceName="communication-svc",
                                serviceVersionPattern="1.76.0*.RELEASE"
                            )
                            ...
                            if (dynamicStagesResults['deploy_communication_svc_passed'] == false) {
                                saveFailedStages("Deploy communication-svc with version 1.76.0*.RELEASE")
                            }
            ...
            steps: {
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        rc_testing.runBDDTests(
                            marks='Empty',
                            test_plan_name='RC [user-svc 1.35.0 | a911-svc 1.133.0 | cms-gw-svc 1.42.0 | hub-svc 1.44.0 | cloud-signaling-svc 1.112.0 | company-svc 1.101.0 | communication-svc 1.76.0]',
                            test_plan_description='RC Testing'
                        )
                    }
                    else {
        ...
        [
            name: "Deploy csa 1.113.0*.RELEASE",
            steps: {
                script {
                    parallel (
                        "csa 1.113.0*.RELEASE": {
                            ...
                            def serviceVersionFromPattern = rc_testing.getLatestServiceVersionByPattern(
                                serviceName="csa",
                                serviceVersionPattern="1.113.0*.RELEASE"
                            )
                            ...
                            if (dynamicStagesResults['deploy_csa_passed'] == false) {
                                saveFailedStages("Deploy csa with version 1.113.0*.RELEASE")
                            }
            ...
            steps: {
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        rc_testing.runBDDTests(
                            marks='Empty',
                            test_plan_name='RC [user-svc 1.35.0 | a911-svc 1.133.0 | cms-gw-svc 1.42.0 | hub-svc 1.44.0 | cloud-signaling-svc 1.112.0 | company-svc 1.101.0 | communication-svc 1.76.0 | csa 1.113.0]',
                            test_plan_description='RC Testing'
                        )
                    }
                    else {
