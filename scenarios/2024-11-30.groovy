...
def getStages() {
    ...
    return [
        [
            name: "Deploy faasfaes afeafaf",
            steps: {
                script {
                    dynamicStagesResults = getDynamicStagesResults()
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        dynamicStagesResults['deploy_faasfaes_passed'] = rc_testing.deployService(
                            serviceName="faasfaes",
                            serviceVersion="afeafaf"
                        )
                    }
                    else {
        ...
        [
            name: "Run BDD tests with marks: fafefaf, faeaf",
            steps: {
                script {
                    dynamicStagesResults = getDynamicStagesResults()
