...
def getStages() {
    ...
    return [
                ...
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        ...
                        if (dynamicStagesResults['deploy_external_device_svc_passed'] == false) {
                          addFailedRCDeploy("external-device-svc", "wrong version")
                        }
                    }
                    else {
                ...
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        rc_testing.runBDDTests(
                            marks='regress',
                            test_plan_name='RC [external-device-svc wrong version]'
                        )
                    }
                    else {
                        def failedStage = dynamicStagesResults.find { stage_passed -> stage_passed.value == false }?.key 
        ...
        [
            name: "Deploy external-device-svc 1.36.1*.RELEASE",
            steps: {
                script {
                    dynamicStagesResults = getDynamicStagesResults()
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        def serviceVersionFromPattern = rc_testing.getLatestServiceVersionByPattern(
                            serviceName="external-device-svc",
                            serviceVersionPattern="1.36.1*.RELEASE"
                        )
