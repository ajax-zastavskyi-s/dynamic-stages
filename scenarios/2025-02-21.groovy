...
def getStages() {
    ...
    return [
                    ...
                    parallel (
                        "a911-svc 1.1.1*.RELEASE": {
                            ...
                            if (dynamicStagesResults['deploy_a911_svc_passed'] == false) {
                                saveFailedDeploy("a911-svc", "1.1.1*.RELEASE")
                            }
        ...
        [
            name: "Restore toggles REDIRECT_RESEND_CONFIRMATION_CODES_TO_USER_SVC []",
            steps: {
                script {
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        parallel (
                        "REDIRECT_RESEND_CONFIRMATION_CODES_TO_USER_SVC []": {
                            dynamicStagesResults = getDynamicStagesResults()
                            ...
                            if (dynamicStagesResults['restore_redirect_resend_confirmation_codes_to_user_svc_passed'] == false) {
                                saveFailedDeploy("REDIRECT_RESEND_CONFIRMATION_CODES_TO_USER_SVC", "")
                            }
                ...
                script {
                    ...
                    if (dynamicStagesResults.every { stage_passed -> stage_passed.value == true }) {
                        rc_testing.runBDDTests(
                            marks='Empty',
                            test_plan_name='RC [a911-svc 1.1.1*.RELEASE]'
                        )
                    }
                    else {
                        def failedStage = dynamicStagesResults.find { stage_passed -> stage_passed.value == false }?.key 
